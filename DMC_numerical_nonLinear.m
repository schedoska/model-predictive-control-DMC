...
U_min = repmat([0;0],Nu,1);
U_max = repmat([50;50],Nu,1);
Y_min = repmat([0;0],N,1);
Y_max = repmat([50;50],N,1);
dU_max = repmat([0.1;0.1],Nu,1);
...
for k = 1:(steps)
    y_zad_v(1:2:N*ny,1) = y_zad(1, k+1:k+N)';
    y_zad_v(2:2:N*ny,1) = y_zad(2, k+1:k+N)';
...
for k = 1:(steps)
    ...
    % du_n = K1*(y_zad_v-repmat([h(k);T(k)],N,1)-Mp*dUp-MPZ*dZp);
    du_n = K1*(repmat(y_zad(:,k),N,1)-repmat([h(k);T(k)],N,1)-Mp*dUp-MPZ*dZp);
    % du_n = K1*(repmat(y_zad(:,k),N,1)-repmat([h(k);T(k)],N,1)-Mp*dUp);
...
legend('DMC 1: h(t), numeryczny','h_{zad}(t)', ...
    ' DMC 2: h(t), analityczny')
set(gca, 'Children', flipud(get(gca, 'Children')) )
subtightplot(2,2,2,gap,margin_h,margin_v)
stairs(czas,wyT,'b','LineWidth',LineWidth);
legend('DMC 1: T(t), numeryczny','T_{zad}(t)', ...
    ' DMC 2: T(t), analityczny')
set(gca, 'Children', flipud(get(gca, 'Children')) )
...
legend('DMC 1: F_h(t), numeryczny',' DMC 2: F_h(t), analityczny')
set(gca, 'Children', flipud(get(gca, 'Children')) )

    % Wykres Sterowania FC
subtightplot(2,2,4,gap,margin_h,margin_v)
stairs(czas,Fc,'Color',[0.424, 0.424, 0.961],'LineWidth',LineWidth);
legend('DMC 1: F_c(t), numeryczny',' DMC 2: F_c(t), analityczny')
set(gca, 'Children', flipud(get(gca, 'Children')) );
