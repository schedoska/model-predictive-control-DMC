...
legend('DMC 1: F_h(t), u^{max}=30, u^{min}=0',' DMC 2: F_h(t), u^{max}=25, u^{min}=21.5')
set(gca, 'Children', flipud(get(gca, 'Children')) )

    % Wykres Sterowania FC
subtightplot(2,2,4,gap,margin_h,margin_v)
stairs(czas,Fc,'Color',[0.424, 0.424, 0.961],'LineWidth',LineWidth);
legend('DMC 1: F_c(t), u^{max}=40, u^{min}=0',' DMC 2: F_c(t), u^{max}=36, u^{min}=31')
set(gca, 'Children', flipud(get(gca, 'Children')) );
