...
for k = 1:(steps)
    ...
    du_n = K1*(repmat(y_zad(:,k)-[h0;T0],N,1)-repmat(x(:,k),N,1)-Mp*dUp);
    ...
    sim('pid_odsp.slx',10000);
    % return

    figure(fig_hT);
subtightplot(2,2,1,gap,margin_h,margin_v)
plot(ans.tout,ans.h_ods+h0,'b','LineWidth',LineWidth);
legend('DMC 1: h(t)','h_{zad}(t)', ...
    'PI: h(t)')
set(gca, 'Children', flipud(get(gca, 'Children')) )
subtightplot(2,2,2,gap,margin_h,margin_v)
plot(ans.tout,ans.T_ods+T0,'b','LineWidth',LineWidth);
...
% figure(fig_FcFh);
subtightplot(2,2,3,gap,margin_h,margin_v)
plot(ans.tout,ans.Fh_ods+FH0,'Color',[0.424, 0.424, 0.961],'LineWidth',LineWidth);
legend('DMC 1: F_h(t)','PI: F_h(t)')
set(gca, 'Children', flipud(get(gca, 'Children')) )

    % Wykres Sterowania FC
subtightplot(2,2,4,gap,margin_h,margin_v)
plot(ans.tout,ans.Fc_ods+FC0,'Color',[0.424, 0.424, 0.961],'LineWidth',LineWidth);
legend('DMC 1: F_c(t)','PI: F_c(t)')
set(gca, 'Children', flipud(get(gca, 'Children')) );
return
